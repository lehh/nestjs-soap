...
const createClientAsyncMock = mocked(createClientAsync).mockResolvedValue(
    // @ts-ignore
    Promise.resolve({
     setSecurity: jest.fn()
    })
...
describe('SoapUtils', () => {
    ...
    test('should create a new client without credentials', async () => {
        ...
        expect(basicAuthSecurityMock).toBeCalledWith(SOAP_MODULE_OPTIONS.auth.username, SOAP_MODULE_OPTIONS.auth.password)
    })
})